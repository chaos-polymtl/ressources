...
\begin{frame}{\textbf{Preamble - Part 1}}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			% \vspace*{-\baselineskip}
			\begin{framed}
				Find a bmtor $\bm{u}$ and a scalar $p$ in $\Omega \rightarrow \mathbb{R}$ satisfying:
				\begin{align*}
					\nabla \cdot \bm{u} &= 0 \\
	...
	\vspace{0.4cm}
	\begin{itemize}
		\item This is a coupled problem for velocity ($\bm{u}$) and pressure ($p$).
...
\begin{frame}
	...
	We multiply by the test functions $\bm{v}$ and $q$, and integrate over the domain using the notation $(a, b)_{\Omega} = \int a b \ \text{d}\Omega$:
	\begin{align*}
		\left( q, \nabla \cdot \bm{u} \right)_{\Omega} + (\bm{v}, \partial_t \bm{u} + (\bm{u} \cdot \nabla) \bm{u} + \underbrace{\nabla p}_{1} - \underbrace{\nu \Delta \bm{u}}_{2} - \bm{f} )_{\Omega} = 0
	\end{align*}
	We integrate by parts terms $1$ and $2$:
	\begin{align*}
		(\bm{v}, \nabla p)_{\Omega} = - (\nabla \cdot \bm{v}, p)_{\Omega} + ( \bm{n} \cdot \bm{v}, p)_{\partial\Omega} \\
		(\bm{v}, -\nu \Delta \bm{u})_{\Omega} = \nu (\nabla \bm{v}, \nabla \bm{u})_{\Omega} - (\bm{v} \bm{n}, \nu \nabla \bm{u})_{\partial\Omega} 
	\end{align*}
	If we assume Dirichlet B.Cs or zero stress conditions on $\partial\Omega$:
	\begin{align*}
		( \bm{n} \cdot \bm{v}, p)_{\partial\Omega} - (\bm{v} \bm{n}, \nu \nabla \bm{u})_{\partial\Omega} = 0
	\end{align*}
	...
	\begin{align*}
		F(\bm{u}, p) := \left( q, \nabla \cdot \bm{u} \right)_{\Omega} + (\bm{v}, \partial_t \bm{u})  + {(\bm{v}, (\bm{u} \cdot \nabla) \bm{u})_{\Omega}} \\
		- (\nabla \cdot \bm{v}, p)_{\Omega} + \nu (\nabla \bm{v}, \nabla \bm{u})_{\Omega} - (\bm{v}, \bm{f})_{\Omega}  = 0
	\end{align*}
	%\visible<2->{
...
\begin{frame}{\textbf{A straightforward implementation}}
	\[
	\pp{q,\partial_t \varepsilon_{\mathrm{f}}}_\Omega + \pp{q, \varepsilon_{\mathrm{f}}\nabla \cdot \vm{u}}_\Omega + \pp{q, \vm{u} \nabla \varepsilon_{\mathrm{f}}}_\Omega + \underbrace{\pp{\tau_{\vm{u}} \nabla q, \vm{\mathcal{R}}^{\mathrm{VANS}}_{\mathrm{A},\mathrm{B}}}_{\Omega_e}}_{\mathrm{PSPG}} = 0
	\]
	\begin{align}
	F^{\mathrm{VANS}}_{\mathrm{A}}\pp{\vm{u}, p, \varepsilon_{\mathrm{f}}} := &\notag \pp{\vm{w}, \varepsilon_{\mathrm{f}}\partial_t\vm{u} + \varepsilon_{\mathrm{f}}\vm{u} \cdot \nabla \vm{u}}_{\Omega} + \pp{\vm{w}, \nu \nabla \vm{u} \cdot \nabla{\varepsilon_{\mathrm{f}}}}_\Omega  \\ 
	\notag &+ \pp{\nabla \vm{w}, \nu \varepsilon_{\mathrm{f}}\nabla \vm{u}}_{\Omega} - \pp{\nabla \cdot \vm{w}, p \varepsilon_{\mathrm{f}}}_\Omega - \pp{\vm{w}, p \nabla \varepsilon_{\mathrm{f}}}_\Omega  \\
	\notag &+ \pp{\vm{w}, \frac{\bar{\vm{f}}_\mathrm{A}^{\mathrm{pf}}}{V_\Omega}}_\Omega + \underbrace{\pp{\tau_{\vm{u}} \vm{u} \cdot \nabla \vm{w}, \vm{\mathcal{R}}^{\mathrm{VANS}}}_{\Omega_e}}_{\mathrm{SUPG}} \\
	 &+ \underbrace{\pp{\gamma \nabla \cdot \vm {w}, \partial_t \varepsilon_{\mathrm{f}}+ \nabla \cdot \pp{\varepsilon_{\mathrm{f}}\vm{u}}}_{\Omega_e}}_{\mathrm{grad-div}} = 0
	 \end{align}
